...
export const PropertyProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        // One-time purge of legacy demo/dummy data from previous templates
        try {
          await Promise.allSettled([
            supabase.from(TABLES.users).delete().in('id', ['usr-admin-1', 'usr-pm-1', 'usr-staff-1', 'usr-tenant-1', 'usr-viewonly-1']),
            supabase.from(TABLES.transferRecords).delete().in('id', ['trf-101', 'trf-102', 'trf-103']),
            supabase.from(TABLES.foodWasteLogs).delete().in('id', ['fwl-101', 'fwl-102', 'fwl-103', 'fw1-101', 'fw1-102', 'fw1-103']),
            supabase.from(TABLES.roomTypes).delete().in('id', ['rtype-1', 'rtype-2', 'rtype-3', 'rtype-4']),
          ]);
        } catch (cleanupErr) {
          console.warn('Dummy cleanup note:', cleanupErr);
        }

        // Ensure essential baseline status categories exist only if table is completely empty
        const { count: statusCount } = await supabase
          .from(TABLES.statuses)
          .select('*', { count: 'exact', head: true });

        if (statusCount === 0 && INITIAL_PROPERTY_DATA.statuses.length > 0) {
          await supabase.from(TABLES.statuses).upsert(
            INITIAL_PROPERTY_DATA.statuses.map(s => cleanForSupabase(statusToRow(s))),
            { onConflict: 'id' }
          );
        }

        // Ensure primary Admin account exists only if users table is completely empty
        const { count: userCount } = await supabase
          .from(TABLES.users)
          .select('*', { count: 'exact', head: true });

        if (userCount === 0 && INITIAL_PROPERTY_DATA.users.length > 0) {
          await supabase.from(TABLES.users).upsert(
            INITIAL_PROPERTY_DATA.users.map(u => cleanForSupabase(userToRow(u))),
            { onConflict: 'id' }
          );
        }
  ...
  const updateBuilding = async (id: string, name: string, code: string, description?: string) => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        buildings: prev.buildings.map((b) => (b.id === id ? { ...b, name, code, description } : b)),
      }));
    } catch (err) {
  ...
  const deleteBuilding = async (id: string, force = false): Promise<boolean> => {
    try {
      ...
      setData((prev) => ({
        ...
        beds: prev.beds.filter((b) => !deletedBedIds.includes(b.id)),
        users: prev.users.map((u) => {
          const nextBedId = u.assignedBedId && deletedBedIds.includes(u.assignedBedId) ? null : u.assignedBedId;
          const nextBuildings = u.assignedBuildingIds ? u.assignedBuildingIds.filter((bId) => bId !== id) : u.assignedBuildingIds;
  ...
  const updateFloor = async (id: string, number: number, label: string, description?: string) => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        floors: prev.floors.map((f) => (f.id === id ? { ...f, number, label, description } : f)),
      }));
    } catch (err) {
    ...
    try {
      ...
      setData((prev) => ({
        ...
        roomTypes: prev.roomTypes.map((rt) =>
          rt.id === id ? { ...rt, name, defaultBedCount, description, badgeColor: badgeColor || rt.badgeColor } : rt
        ),
      }));
    } catch (err) {
  ...
  const deleteRoomType = async (id: string, force = false): Promise<boolean> => {
    try {
      ...
      const fallbackTypeId = fallbackType?.id || 'rtype-std';

      if (supabaseMaybe) {
        if (associatedRooms.length) {
          await supabase.from(TABLES.rooms)
            .update({ room_type_id: fallbackTypeId })
      ...
      setData((prev) => ({
        ...
        rooms: prev.rooms.map((r) => (r.roomTypeId === id ? { ...r, roomTypeId: fallbackTypeId } : r)),
      }));
        ...
        statuses: prev.statuses.map((s) =>
          s.id === id
            ? {
                ...
                color,
                description,
                isOccupiedState: isOccupiedState ?? s.isOccupiedState,
                isMaintenanceState: isMaintenanceState ?? s.isMaintenanceState,
              }
            : s
        ),
      }));
  ...
  const updateRoom = async (
    ...
    try {
      ...
      if (totalBeds > currentBeds.length) {
        ...
        removedBedIds = currentBeds.slice(totalBeds).map((b) => b.id);
      }

      if (supabaseMaybe) {
        if (createdBeds.length) {
          const { error } = await supabase.from(TABLES.beds).insert(createdBeds.map((b) => cleanForSupabase(bedToRow(b))));
          if (error) throw error;
        } else if (removedBedIds.length) {
          const { error } = await supabase.from(TABLES.beds).delete().in('id', removedBedIds);
          if (error) throw error;
        }
      ...
      setData((prev) => ({
        ...
        beds: [
          ...prev.beds.filter((b) => !removedBedIds.includes(b.id)),
          ...createdBeds,
        ],
      }));
    } catch (err) {
  ...
  const updateBedStatus = async (bedId: string, statusId: string, notes?: string) => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        beds: prev.beds.map((b) => (b.id === bedId ? { ...b, statusId, notes: notes ?? b.notes } : b)),
        rooms: prev.rooms.map((r) =>
          r.id === targetBed.roomId ? { ...r, statusId: calcRoomStatus, updatedAt: new Date().toISOString() } : r
        ),
      }));
  ...
  const addMaintenanceRequest = async (reqData: {
    ...
    try {
      ...
      setData((prev) => ({
        ...prev,
        maintenanceRequests: [newRequest, ...(prev.maintenanceRequests || [])],
        beds: reqData.setRoomBedMaintenance && reqData.bedId
          ? prev.beds.map((b) => (b.id === reqData.bedId ? { ...b, statusId: 'status-maintenance' } : b))
          : prev.beds,
        rooms: reqData.setRoomBedMaintenance && reqData.roomId
          ? prev.rooms.map((r) => (r.id === reqData.roomId ? { ...r, statusId: 'status-maintenance' } : r))
          : prev.rooms,
      }));
    } catch (err) {
  ...
  const completeMaintenanceRequest = async (id: string, resolutionNotes?: string, revertRoomBedStatus: boolean = true) => {
    try {
      ...
      if (supabaseMaybe) {
        ...
        if (revertRoomBedStatus) {
          if (targetReq.bedId && nextBedStatus) {
            await supabase.from(TABLES.beds).update({ status_id: nextBedStatus }).eq('id', targetReq.bedId);
          }
          if (targetReq.roomId && nextRoomStatus) {
            await supabase.from(TABLES.rooms).update({ status_id: nextRoomStatus }).eq('id', targetReq.roomId);
          }
        }
      ...
      setData((prev) => ({
        ...
        beds: nextBedStatus && targetReq.bedId
          ? prev.beds.map((b) => (b.id === targetReq.bedId ? { ...b, statusId: nextBedStatus } : b))
          : prev.beds,
        rooms: nextRoomStatus && targetReq.roomId
          ? prev.rooms.map((r) => (r.id === targetReq.roomId ? { ...r, statusId: nextRoomStatus } : r))
          : prev.rooms,
      }));
    } catch (err) {
  ...
  const addTransferRecord = async (recordData: Omit<TransferRecord, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        transferRecords: [newRecord, ...(prev.transferRecords || [])],
      }));
    } catch (err) {
  ...
  const updateTransferRecord = async (id: string, updates: Partial<TransferRecord>) => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        transferRecords: (prev.transferRecords || []).map((t) =>
          t.id === id
            ? {
                ...t,
                ...updates,
                updatedAt: now,
                lastModifiedBy: modifier,
                auditLogs: updatedLogs || t.auditLogs,
              }
            : t
        ),
      }));
  ...
  const deleteTransferRecord = async (id: string) => {
    try {
      ...
      setData((prev) => ({
        ...prev,
        transferRecords: (prev.transferRecords || []).filter((t) => t.id !== id),
      }));
    } catch (err) {
